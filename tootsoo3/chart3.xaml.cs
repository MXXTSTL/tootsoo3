...
{
    /// <summary>
    /// Interaction logic for chart3.xaml
    /// </summary>
    public partial class chart3 : UserControl
    {
        ...
        private void LoadDataAndRenderChart()
        {
            try
            {
                ...
                List<DateTimePoint> seriesUndur5 = new List<DateTimePoint>();
                ...
                SELECT u.*
                    ...
                    if (dsundur.Tables.Count > 0 && dsundur.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow row in dsundur.Tables[0].Rows)
                        {
                            ...
                            double undur1Value = Convert.ToDouble(row["tg5"]);
                            double undur2Value = Convert.ToDouble(row["tg6"]);
                            double undur3Value = Convert.ToDouble(row["tg7"]);
                            double undur4Value = Convert.ToDouble(row["tg8"]);
                            double undur5Value = Convert.ToDouble(row["tg9"]);
                            double undur6Value = Convert.ToDouble(row["niilber"]);
                            ...
                            seriesUndur5.Add(new DateTimePoint(tsag, undur5Value));
                            seriesUndurNiit.Add(new DateTimePoint(tsag, undur6Value));
                        }
                    }
                }
